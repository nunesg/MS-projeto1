...
class StatisticsManager:
  ...
  def get_client(self, idx):
    if idx > self.nclients:
      raise Exception(f"Client index {idx} is out of range {self.nclients}")
  ...
  def print_statistics(self, servers):
    ...
    # b)
    avg_total_idle_time = sum(server['server'].get_total_idle_time() for server in servers)/len(servers)
